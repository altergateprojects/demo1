-- Fix Fee Payment System to Handle Previous Years Pending
-- This creates a proper payment allocation function

-- First, create a function to handle payment allocation
...
-- Create a simpler wrapper function for the API
...
CREATE POLICY "Users can record fee payments" ON fee_payments
...
COMMENT ON FUNCTION process_fee_payment_with_allocation IS 'Intelligently allocates fee payments: first to previous years debt, then current year, then pocket money';
COMMENT ON FUNCTION record_fee_payment_smart IS 'Wrapper function for fee payment allocation with authentication';